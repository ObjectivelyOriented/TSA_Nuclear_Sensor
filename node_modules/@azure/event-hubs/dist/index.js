...
/** @internal */
const receiverIdPropertyName = "com.microsoft:receiver-name";
/**
 * @internal
 */
...
function isAmqpAnnotatedMessage(possible) {
    return (coreUtil.isObjectWithProperties(possible, ["body", "bodyType"]) &&
        !coreUtil.objectHasProperty(possible, "getRawAmqpMessage"));
}
/**
...
function convertDatesToNumbers(thing) {
    ...
    if (typeof thing === "object" &&
        coreUtil.objectHasProperty(thing, "getTime") &&
...
async function withAuth(callback, context, audience, timeoutInMs, logger, options) {
    ...
    async function createTask() {
        try {
            await setupClaimNegotiation(context, audience, info, timeoutInMs, logger, options);
            logger.verbose(`next token renewal is in ${info.timeoutInMs} milliseconds @(${new Date(Date.now() + info.timeoutInMs).toString()}).`);
        }
        catch (err) {
...
async function openCbsSession(client, timeoutAfterStartTime, { abortSignal } = {}) {
    return coreAmqp.defaultCancellableLock.acquire(client.cbsLock, () => client.init({
        abortSignal,
...
function getSharedKeyBasedToken(cred, audience) {
    return {
...
function createReceiver(ctx, consumerGroup, consumerId, partitionId, eventPosition, options = {}) {
    ...
    const logger$1 = createSimpleLogger(logger, logPrefix);
    const queue = [];
    const state = {
        isConnecting: false,
    };
    const obj = {
        ...
        close: async () => {
            ...
            logger$1.verbose("deleted the receiver from the client cache");
            (_a = state.authLoop) === null || _a === void 0 ? void 0 : _a.stop();
            return (_b = state.link) === null || _b === void 0 ? void 0 : _b.close().catch((err) => {
                logger$1.warning(`an error occurred while closing: ${err === null || err === void 0 ? void 0 : err.name}: ${err === null || err === void 0 ? void 0 : err.message}`);
                logErrorStackTrace(err);
                throw err;
            }).finally(() => {
                obj.isClosed = true;
                logger$1.verbose("is closed");
        ...
        abort: () => {
            ...
            logger$1.info(abortLogMessage);
            return obj.close();
        },
        isOpen: () => {
            var _a;
            const isOpen = !!((_a = state.link) === null || _a === void 0 ? void 0 : _a.isOpen());
            logger$1.verbose(`is open? -> ${isOpen}`);
            return isOpen;
        },
        async connect({ abortSignal, timeoutInMs, prefetchCount }) {
            if (state.isConnecting || obj.isOpen()) {
                return;
            }
            state.isConnecting = true;
            logger$1.verbose("is trying to connect");
            try {
                await ctx.readyToOpenLink({ abortSignal });
                state.authLoop = await withAuth(() => setupLink(consumerId, ctx, name, address, obj, state, queue, prefetchCount, eventPosition, logger$1, options, abortSignal), ctx, audience, timeoutInMs, logger$1, {
                    abortSignal,
                });
            }
            catch (err) {
                ...
                logger$1.error(`an error occurred while creating the receiver: ${error === null || error === void 0 ? void 0 : error.name}: ${error === null || error === void 0 ? void 0 : error.message}`);
                logErrorStackTrace(err);
                throw error;
            }
        },
        receiveBatch: (maxMessageCount, maxWaitTimeInSeconds = 60, abortSignal) => {
            ...
            const retrieveEvents = () => {
                ...
                logger$1.verbose(`already has ${queue.length} events and wants to receive ${eventsToRetrieveCount} more events`);
                ...
                return obj.isClosed || ctx.wasConnectionCloseCalled || eventsToRetrieveCount === 0
                    ...
                    : new Promise((resolve, reject) => {
                        ...
                        obj // eslint-disable-line promise/catch-or-return
                            ...
                            prefetchCount: (_a = options.prefetchCount) !== null && _a !== void 0 ? _a : maxMessageCount * 3,
                        })
                            .then(() => {
                            logger$1.verbose(`setting the wait timer for ${maxWaitTimeInSeconds} seconds`);
                            return waitForEvents(maxMessageCount, maxWaitTimeInSeconds * 1000, qReadIntervalInMs, queue, {
                                abortSignal,
                                cleanupBeforeAbort,
                                receivedAfterWait: () => logger$1.info(`${Math.min(maxMessageCount, queue.length)} messages received within ${maxWaitTimeInSeconds} seconds`),
                                receivedAlready: () => logger$1.info(`${maxMessageCount} messages already received`),
                                receivedNone: () => logger$1.info(`no messages received when max wait time in seconds ${maxWaitTimeInSeconds} is over`),
                            });
                        })
                            .catch(reject)
                            .then(resolve);
            ...
            return coreAmqp.retry(Object.defineProperties({
                ...
                retryOptions: (_a = options.retryOptions) !== null && _a !== void 0 ? _a : {},
            }, {
                connectionId: {
                    enumerable: true,
...
function createRheaOptions(consumerId, name, address, obj, state, queue, prefetchCount, eventPosition, logger, options) {
    const rheaOptions = {
        name,
        autoaccept: true,
        target: consumerId,
...
async function setupLink(consumerId, ctx, name, address, obj, state, queue, prefetchCount, eventPosition, logger, options, abortSignal) {
    const rheaOptions = createRheaOptions(consumerId, name, address, obj, state, queue, prefetchCount, eventPosition, logger, options);
...
function isEventDataBatch(eventDataBatch) {
    return (coreUtil.isObjectWithProperties(eventDataBatch, ["count", "sizeInBytes", "tryAdd"]) &&
...
function generateIdempotentLinkProperties(userProvidedPublishingOptions, localPublishingOptions) {
    ...
    const idempotentLinkProperties = {
        [idempotentProducerAmqpPropertyNames.epoch]: coreUtil.isDefined(ownerLevel)
            ? rheaPromise.types.wrap_short(ownerLevel)
            : null,
        [idempotentProducerAmqpPropertyNames.producerId]: coreUtil.isDefined(producerGroupId)
            ? rheaPromise.types.wrap_long(producerGroupId)
            : null,
        [idempotentProducerAmqpPropertyNames.producerSequenceNumber]: coreUtil.isDefined(sequenceNumber)
...
class EventHubProducerClient {
    ...
    async createBatch(options = {}) {
        var _a;
        throwErrorIfConnectionClosed(this._context);
        const partitionId = coreUtil.isDefined(options.partitionId) ? String(options.partitionId) : undefined;
        ...
        if (!sender) {
            const partitionPublishingOptions = coreUtil.isDefined(partitionId)
    ...
    async sendBatch(batch, options = {}) {
        ...
        if (isEventDataBatch(batch)) {
            if (this._enableIdempotentRetries &&
                coreUtil.isDefined(batch.startingPublishedSequenceNumber)) {
